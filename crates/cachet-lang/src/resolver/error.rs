...
#[derive(Clone, Debug, Error)]
#[error("can't nest {} under {parent_kind} `{parent}`", OptionalItemIdent(.ident))]
        ...
        Diagnostic::error()
            ...
            .with_labels(vec![
                Label::primary(file_id, self.span()).with_message(format!(
                    "items of this kind can't be nested under {} items",
                    self.parent_kind
                )),
    ...
    fn build_diagnostic<T: Copy>(&self, file_id: T) -> Diagnostic<T> {
        Diagnostic::error()
            ...
            .with_labels(vec![Label::primary(file_id, self.span()).with_message(
                "items of this kind must be nested under another item",
            )])
    }
}
